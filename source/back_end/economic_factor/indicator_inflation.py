from econ_indicator_sanity_api import InflationSanity
import json
...
class Inflation:
    def __init__(self, interval: str = 'annual'):
        """
        Initializes an Inflation instance for the given interval.
        Valid intervals: 'annual'.
        Retrieves data using InflationSanity and stores it for later use.
        """
        self._sanity_data = InflationSanity(interval).get_data()
    ...
    def get_data(self) -> str:
        """
        Returns the inflation data in JSON format.
        """
        return json.dumps({
            "name": self.name,
